(function() {
    ...
    request({url:playlist_url, headers:{"Authorization":access_token}}, function(err,res){ //err = error, res = result 
        if(res){
            var playlists=JSON.parse(res.body);
            varplaylist_url = playlists.item[0].href
            request({url:playlist_url, headers:{"Authorization":access_token}}, function(err,res){
                if(res){
